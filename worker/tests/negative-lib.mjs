...
// 2 = the break did not apply (the anchor text moved), which is also a failure, never a silent pass.
...
export async function negative({ name, why, patches, args, expectRed, port = 7805 }) {
  ...
  const summary = lines.filter((l) => /^(# (tests|pass|fail)\b|== (api|unit):)/.test(l))
  const result = red
