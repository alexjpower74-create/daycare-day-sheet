// `npm test`: pure unit tests, then the API suite against a fresh local Worker. Adapted from Firewood Orders.
//   PORT (default 7802, inspector PORT+10). State in worker/.state-<PORT>, wiped first.
...
if (!flag('--unit-only')) {
  ...
  if (await answers()) {
    ...
    const m = spawnSync('wrangler', ['d1', 'migrations', 'apply', 'daycare-day-sheet', '--local', '--persist-to', STATE],
