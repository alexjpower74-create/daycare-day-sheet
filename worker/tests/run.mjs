...
// fresh Worker WITHOUT TEST_MODE. Adapted from Firewood Orders.
//   PORT (default 7802, inspector PORT+10). State in worker/.state-<PORT> (and .state-<PORT>-setup), wiped first and removed after.
...
// A made-up centre for the first-setup check only (no SAMPLE rows on purpose: that is what the check proves).
const SETUP = { centre: 'First Setup Check Centre', phone: '709-555-0150', supervisor: 'Setup Check Supervisor', pin: '5827' }
...
function freshState(dir) {
  ...
  return wrangler(['d1', 'migrations', 'apply', 'daycare-day-sheet', '--local', '--persist-to', dir])
}

async function startWorker(dir, testVars) {
...
async function stopWorker(dev) {
  try { process.kill(-dev.pid, 'SIGTERM') } catch {}
  // Wait until the port is really free, so the next stage cannot talk to this Worker.
  const t0 = Date.now()
...
if (!flag('--api-only')) {
  const unit = readdirSync(path.join(WORKER, 'tests')).filter((f) => f.endsWith('.test.mjs') && !f.startsWith('api'))
...
if (!flag('--unit-only')) {
  ...
  if (await answers()) {
    ...
    dev = await startWorker(STATE, true)
    if (!dev) {
  ...
  if (grep) {
    ...
    ok = ok && spawnSync(process.execPath, ['tools/first-setup.mjs', '--centre', SETUP.centre, '--phone', SETUP.phone,
      '--supervisor', SETUP.supervisor, '--pin', SETUP.pin, '--out', sqlFile], { cwd: WORKER, stdio: 'inherit' }).status === 0
    ok = ok && wrangler(['d1', 'execute', 'daycare-day-sheet', '--local', '--persist-to', SSTATE, '--file', sqlFile])
    ...
    if (!setupDev) {
      ...
      failed |= runNodeTests(['tests/api-setup.test.mjs'], [], {
        SETUP_BASE: BASE, SETUP_CENTRE: SETUP.centre, SETUP_PHONE: SETUP.phone, SETUP_SUPERVISOR: SETUP.supervisor, SETUP_PIN: SETUP.pin,
      })
